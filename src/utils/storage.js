...
export function displayScores() {
    ...
    const scores = JSON.parse(localStorage.getItem(config.scoresStorageKey)) || [];
    
    const attempts = scores.length > 0 ? scores.map((_, index) => index + 1) : [0];
    const wpmData = scores.length > 0 ? scores.map(score => score.wordCount) : [0];
    const accuracyData = scores.length > 0 ? scores.map(score => typeof score.accuracy === 'number' ? score.accuracy.toFixed(2) : '0.00') : [0];
...
document.getElementById('showScoresLink').addEventListener('click', function(event) {
    event.preventDefault();
    const scoresChartContainer = document.getElementById('scoresChartContainer');
    if (scoresChartContainer) { // Ensure scoresChartContainer is available
...
export function saveScore(wordCount, accuracy) {
    ...
    const improvementMessage = scores.length > 1 ?
        `Previous Score: ${scores[scores.length - 2].wordCount} words, ${scores[scores.length - 2].accuracy.toFixed(2)}% accuracy. Current Score: ${wordCount} words, ${accuracyText}% accuracy.` :
    ...
    if (!document.getElementById('scoresChartContainer').classList.contains('d-none')) {
        displayScores();
    }
}
